...
def generateSystem(simulation = ttor,seed = None, asteroidnumber = 1000):  
    sim = simulation()
    sim.N_active = sim.N
...
#     T=80*2*np.pi #years of simulation
    ...
    .format(seed,0,0/tau,sim.N,0,round((tiempo.monotonic()-initialtime),1)\
    ,round((tiempo.monotonic()-initialtime)/60,1)))
    #
    for i, t in enumerate(times):
        sim.integrate(t)
        print(" nor seed: {} | {} time = {} years | {} particles | {} step number |\n\
...
#     print("There are {} particles remaining.".format(sim.N))
    #
    print("seed: {} | {} time = {} years | {} particles | {} step number |\n| {} second | {} minutes | {} hours.\n"\
    .format(seed,sim.t,sim.t/tau,sim.N,stepnumber,round((tiempo.monotonic()-initialtime),1)\
    ,round((tiempo.monotonic()-initialtime)/60,1), round((tiempo.monotonic()-initialtime)/3600,1)))
    #
    quickplot(sim)
    #ding()
...
sim = generateSystem(simulation = nor, seed =a, asteroidnumber = 2000)
...
masslist_txt_append(ttor_masses,'Masslists/2000July21NOR.txt','ttor','a', first = first, last = last, lastN = lastN)
print(ttor_masses)
print("There are {} particles remaining.".format(sim.N))

saveFigs(innerFolder= "2000asteroidsJuly21NOR",seed = a) # the folder witin the figures folder is set with the seed kwarg. Setting seed = "Tests" will
